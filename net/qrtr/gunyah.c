				 ...
				 gh_vmid_t peer)
{
	...
	if (ret) {
		pr_err("%s: qcom_scm_assign_mem failed addr=0x%llx size=%zu err=%d\n",
		       __func__, (unsigned long long)qdev->res.start, qdev->size, ret);
		return ret;
	}
	...
	if (ret) {
		pr_err("%s: gh_rm_mem_share failed addr=0x%llx size=%zu err=%d\n",
		       __func__, (unsigned long long)qdev->res.start, qdev->size, ret);
				    ...
				    gh_vmid_t self, gh_vmid_t peer)
{
	...
	if (ret)
		pr_err("%s: qcom_scm_assign_mem failed addr=0x%llx size=%llu err=%d\n",
		       __func__, (unsigned long long)qdev->res.start, (unsigned long long)resource_size(&qdev->res), ret);
}
