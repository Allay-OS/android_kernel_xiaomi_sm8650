			   ...
			   gh_vmid_t peer)
{
	...
	if (ret) {
		dev_err(qdd->dev, "qcom_scm_assign_mem addr=0x%llx size=%llu failed: %d\n",
		       (unsigned long long)qdd->res.start, (unsigned long long)qdd->size, ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(qdd->dev, "Gunyah mem share addr=0x%llx size=%llu failed: %d\n",
		       (unsigned long long)qdd->res.start, (unsigned long long)qdd->size, ret);
