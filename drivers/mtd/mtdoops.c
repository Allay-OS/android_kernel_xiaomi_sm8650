			    ...
			    enum mtd_dump_reason reason)
{
	...
	if ((int)reason == (int)KMSG_DUMP_SHUTDOWN ||
		(int)reason == (int)KMSG_DUMP_EMERG)
		return;

	/* Only dump oopses if dump_oops is set */
	if ((int)reason == (int)KMSG_DUMP_OOPS && !dump_oops)
		return;
	...
	if ((int)reason == (int)KMSG_DUMP_OOPS || (int)reason == (int)KMSG_DUMP_PANIC) {
